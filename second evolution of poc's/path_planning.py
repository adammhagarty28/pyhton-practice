...
t_end         = 25.0
steps         = int(t_end / dt)
steps_per_move = 50       # timesteps between nozzle moves
...
T_init = T_init.at[0,  :].set(T_ambient)
T_init = T_init.at[-1, :].set(T_ambient)
T_init = T_init.at[:,  0].set(T_ambient)
T_init = T_init.at[:, -1].set(T_ambient)
...
def step(carry, _):
    ...
    # distance penalty: bias toward hot nodes far from current nozzle
    ...
    # --- spray mask for current nozzle position ---
    ...
    T_new = T_new.at[0,  :].set(T_ambient)
    T_new = T_new.at[-1, :].set(T_ambient)
    T_new = T_new.at[:,  0].set(T_ambient)
    T_new = T_new.at[:, -1].set(T_ambient)
